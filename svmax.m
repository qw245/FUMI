...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         if SNR < SNR_th,   
                if strcmp (verbose, 'on'), fprintf(1,'... Select the orthogonal proj. to p-1 \n',SNR); end
                ...
                if strcmp (verbose, 'on'), fprintf(1,'... Select projective proj. to p\n',SNR); end
...
%                c = max(sum((y).^2))^0.5; % so ca esta por causa do codiggo seguinte
%                y = [y ; c*ones(1,N)] ;
...
A = zeros(p,p);
A(p,1) = 1;
          [v_max indice(1)] = max(sum(y.^2));
           A(:,1) = y(:,indice(1));        % same as x(:,indice(1))
...
%          P_x = mean(x(:).^2)*p;
%          P_m = r_m'*r_m;
%          varEst = (P_y-P_x-P_m)/(L-p);
%          snr_est = (P_x+P_m-varEst*p)/varEst/L;
